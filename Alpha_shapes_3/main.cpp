...
int main()
{
    ...
    if(type=="unprocessed"){
        for (size_t i = 0; i < input_files1.size(); i++) {
            ...
            // make a new folder to store alpha shape in .off format
            std::string export_dir="C:/Users/seuya/Documents/Thesis/Intermediate_Data/OFF/"+type+"/"+std::to_string(grid_size)+"/";
            ...
            // make a new folder to store alpha shape in .obj format
            std::string export_obj_dir="C:/Users/seuya/Documents/Thesis/Intermediate_Data/OBJ/"+type+"/"+std::to_string(grid_size)+"/";
    ...
    else if(type=="wlop_simplify"){

        // wlop simplify, point cloud simplification
    ...
    std::vector<std::string> processed= GetInputs(processed_dir);
    // alpha shape
    for (size_t i = 0; i < processed.size(); i++) {
        ...
        alpha_shape_constructor(processed[i],export_path,out_v,out_f,alpha);
        ...
        for (size_t i = 0; i < input_files3.size(); i++) {
            ...
            std::string simplified_obj = mesh_simplify_dir+ clear_slash(input_files3[i])+ ".obj";
